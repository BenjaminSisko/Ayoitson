...
type RequestLike = { method: string; path: string };
type ResponseLike = AnyRecord;
type NextLike = (...args: any[]) => void;

const nodeVersionMatch = process.version.match(/^[^0-9]*(\d+)\..*$/);
const NODE = parseInt(nodeVersionMatch?.[1] ?? '0', 10);

if (NODE < 12) {
  console.error(
    `WARNING: Your nodejs version ${process.version} is lower than supported. ${constants.APP_NAME} has been tested best on nodejs 12.16.`
  );
}
...
if (
  ...
  console.warn(
    'WARNING: DATABASE is deprecated; use AYOITSON_DATABASE. DATABASE will be removed after one compatibility release.'
  );
}
const runtimeDirs = resolveRuntimeDataDirs({
  ...
  legacyDir:
    ...
    (cliDatabaseDir || deprecatedDatabaseDir
      ? undefined
      : path.join('.', '.dizquetv')),
});
...
const databaseDir = process.env.AYOITSON_DATABASE || runtimeDirs.databaseDir;
...
const fillerDB = new FillerDAO(db.sqlite, channelService);
const customShowDB = new CustomShowDAO(db.sqlite);
...
const fileCache = new FileCacheService(path.join(databaseDir, 'cache'));
...
const activeChannelService = new ActiveChannelService(
  onDemandService,
  channelService
);
...
guideService.on('xmltv-updated', (_data: any) => {
  try {
...
app.get('/version.js', (_req: any, res: any) => {
  res.writeHead(200, {
...
app.use(
  '/cache/images',
  express.static(path.join(databaseDir, 'cache', 'images'))
);
app.use(
  '/favicon.svg',
...
function isReactSpaPath(reqPath: string): boolean {
  ...
  return reservedPaths.every(
    (reserved) => reqPath !== reserved && !reqPath.startsWith(`${reserved}/`)
  );
}

function sendReactSpaIndex(res: ResponseLike, next: NextLike) {
  fs.readFile(
    path.join(reactDistDir, 'index.html'),
    'utf8',
    (err: NodeJS.ErrnoException | null, html: string) => {
      if (err) {
        next(err);
        return;
      }
      res
        .type('html')
        .send(
          html.replace(/__AYOITSON_CSP_NONCE__/g, res.locals.cspNonce || '')
        );
    }
  );
}
...
app.get('*', (req: RequestLike, res: ResponseLike, next: NextLike) => {
