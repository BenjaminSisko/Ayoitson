...
function createRouter(deps) {
  ...
  router.get(
    '/last-refresh',
    asyncRoute(async (req, res) => {
      const lastUpdated = xmltvInterval && xmltvInterval.lastUpdated
  ...
  router.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      const lineup = await guideService.getChannelLineup(number, dateFrom, dateTo);
      if (lineup == null) {
        return apiError(res, NOT_FOUND, 'Channel not found in TV guide', { number });
      }
      res.send(lineup);
    })
  );
    ...
    asyncRoute(async (req, res) => {
      ...
      try {
        ...
        if (err && err.code === 'ENOENT') {
          ...
          // envelope is JSON.
          return apiError(res, NOT_FOUND, 'xmltv.xml has not been generated yet');
        }
        throw err;
      }
