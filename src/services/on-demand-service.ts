...
class OnDemandService {
  ...
  async registerChannelStopped(channelNumber, stopTime, waitForSave) {
    try {
      ...
      if (channel == null) {
        console.error(
          'Could not stop channel ' +
            channelNumber +
            ' because it apparently no longer exists'
        ); // I guess if someone deletes the channel just in the grace period?
        return;
      }

      if (
        typeof channel.onDemand !== 'undefined' &&
        channel.onDemand.isOnDemand &&
        !channel.onDemand.paused
      ) {
        //pause the channel
  ...
  pauseOnDemandChannel(originalChannel, stopTime) {
    ...
    if (channel.programs.length == 0) {
      console.log(
        "On-demand channel has no programs. That doesn't really make a lot of sense..."
      );
  ...
  async updateChannelSync(channel) {
    try {
      await this.channelService.saveChannel(channel.number, channel, {
        ignoreOnDemand: true,
      });
      console.log(
        'Channel ' + channel.number + ' saved by on-demand service...'
      );
    } catch (err) {
  ...
  fixupChannelBeforeSave(channel) {
    ...
    if (
      this.activeChannelService != null &&
      this.activeChannelService.isActive(channel.number)
    ) {
      isActive = true;
    }
  ...
  resumeOnDemandChannel(t, originalChannel) {
    ...
    if (programs.length == 0) {
      console.log(
        "On-demand channel is empty. This doesn't make a lot of sense..."
      );
      return channel;
    }
    let i = 0;
    ...
    while (i < programs.length) {
      ...
      if (program.isOffline && program.type !== 'redirect') {
        ...
        onDemand.firstProgramModulo =
          (onDemand.firstProgramModulo + program.duration) % onDemand.modulo;
      } else {
        break;
      }
    }
    if (i == programs.length) {
      console.log(
        "Everything in the channel is flex... This doesn't really make a lot of sense for an onDemand channel, you know..."
      );
      i = 0;
      onDemand.playedOffset = 0;
    ...
    let pm =
      (onDemand.firstProgramModulo + onDemand.playedOffset) % onDemand.modulo;
