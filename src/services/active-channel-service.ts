...
class ActiveChannelService {
  ...
  loadChannelsForFirstTry() {
    let fun = async () => {
      try {
        ...
        console.error(
          'Unexpected error when checking channels for the first time.',
          err
        );
      }
    };
  ...
  async shutdown() {
    try {
      ...
      for (const [channelNumber, value] of Object.entries(this.cache)) {
        console.log(
          'Forcefully registering channel ' + channelNumber + ' as stopped...'
        );
        delete this.cache[channelNumber];
        await this.onDemandService.registerChannelStopped(
          channelNumber,
          t,
          true
        );
      ...
      console.error(
        'Unexpected error when shutting down active channels service.',
        err
      );
    }
  }
  ...
  setupTimer() {
    this.handle = setTimeout(
      () => this.timerLoop(),
      constants.PLAYED_MONITOR_CHECK_FREQUENCY
    );
  }
  ...
  checkChannel(t, channelNumber, value) {
    if (value.active === 0) {
      ...
      if (
        delta >= constants.MAX_CHANNEL_IDLE ||
        value.lastUpdate <= this.timeNoDelta
      ) {
        console.log('Channel : ' + channelNumber + ' is not playing...');
        this.onDemandService.registerChannelStopped(
          channelNumber,
          value.stopTime
        );
        delete this.cache[channelNumber];
  ...
  registerChannelStopped(t, channelNumber) {
    ...
    if (this.cache[channelNumber].active === 1) {
      console.log(
        'Register that channel is no longer being played: ' + channelNumber
      );
    }
    if (this.cache[channelNumber].active === 0) {
