  ...
  async makeAccumulated(channel) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!Number.isInteger(d)) {
        console.log(
          `Found program in channel ${channel.number} with non-integer duration ${d}, correcting it`
        );
        d = Math.ceil(d);
      }
  ...
  async getCurrentPlayingIndex(channel, t) {
    ...
    if (t < s) {
      ...
      if (lo < 0 || lo >= channel.programs.length || accumulate[lo + 1] <= d) {
        console.log(
          '[tv-guide] The binary search algorithm is messed up. Replacing with flex...'
        );
        return {
          index: -1,
          start: t,
    ...
    if (playing.program.isOffline && playing.program.type === 'redirect') {
      ...
      if (depth.indexOf(ch2) != -1) {
        ...
        if (typeof channel2 === 'undefined') {
          ...
          if (!(a1 <= t && t < b1)) {
            console.error(
              '[tv-guide] algorithm error1 : ' + a1 + ', ' + t + ', ' + b1
            );
          }
          if (!(a2 <= t && t < b2)) {
            console.error(
              '[tv-guide] algorithm error2 : ' + a2 + ', ' + t + ', ' + b2
            );
          }
  ...
  async getChannelPrograms(t0, t1, channel) {
    ...
    while (x.start < t1) {
      ...
      if (x.program.duration == 0) {
        console.error(
          channel.number +
            " There's a program with duration 0? " +
            JSON.stringify(x.program) +
            ' ; ' +
            t1
        );
        ...
        console.error(
          channel.number +
            " There's a program with non-integer duration?? " +
            JSON.stringify(x.program) +
            ' ; ' +
            t1
        );
