...
function video(
  ...
  router.get('/setup', (req, res) => {
    ...
    let ffmpeg = new FFMPEG_TEXT(
      ffmpegSettings,
      'Ayoitson (No Channels Configured)',
      'Configure your channels using the Ayoitson Web UI'
    );
  ...
  let concat = async (req, res, audioOnly, step) => {
    ...
    ffmpeg.on('end', () => {
      console.log(
        'Queue exhausted so we are appending the channel stream again to the http output.'
      );
    ...
    let ff = await ffmpeg.spawnConcat(
      `${baseUrl}/playlist?channel=${channelNum}&audioOnly=${audioOnly}&stepNumber={step}`
    );
  ...
  let streamFunction = async (req, res, t0, allowSkip) => {
    ...
    let isBetween =
      typeof req.query.between !== 'undefined' && req.query.between == '1';
    ...
    if (lineupItem != null) {
      ...
      while (true) {
        ...
        if (lineupItem != null) {
          ...
          prog = programmingService.getCurrentProgramAndTimeElapsed(
            t0,
            newChannel
          );
    ...
    if (typeof lineupItem.streamDuration === 'undefined') {
      ...
      console.log(
        `! From : ${lineupItem.start} to: ${lineupItem.start + lineupItem.streamDuration}`
      );
    }
    console.log('=========================================================');
    ...
    shieldActiveChannels(
      redirectChannels,
      t0,
      constants.START_CHANNEL_GRACE_PERIOD
    );
    ...
    if (!isLoading) {
      ...
      for (let i = redirectChannels.length - 1; i >= 0; i--) {
        activeChannelService.registerChannelActive(
          t0,
          redirectChannels[i].number
        );
      }
      let listener = (data) => {
        if (data.ignoreOnDemand) {
          console.log(
            'Ignore channel update because it is from on-demand service'
          );
          return;
        }
        ...
        try {
          ...
          if (shouldStop) {
            ...
            shieldActiveChannels(
              redirectChannels,
              t0,
              constants.CHANNEL_STOP_SHIELD
            );
            setTimeout(stop, 100);
          }
        } catch (error) {
          console.err(
            'Unexpected error when processing channel change during playback',
            error
          );
        }
      };
      ...
      stop = () => {
        ...
        if (!stopDetected) {
          ...
          for (let i = redirectChannels.length - 1; i >= 0; i--) {
            activeChannelService.registerChannelStopped(
              t1,
              redirectChannels[i].number
            );
          }
