...
class PlexTranscoder {
  ...
  async plexGet(url, headers = { Accept: 'application/json' }) {
    const response = await httpGet(url, this.getHttpOptions(headers));
    if (!response.ok) {
      throw Error(`Plex GET returned status code ${response.status}`);
    }
    return response.data;
  }

  async plexPost(url, headers = { Accept: 'application/json' }) {
    const response = await httpPost(url, this.getHttpOptions(headers));
    if (!response.ok) {
      throw Error(`Plex POST returned status code ${response.status}`);
    }
    return response.data;
  }
...
X-Plex-Token=${this.server.accessToken}`;
  ...
  updatePlex() {
    ...
    const updatePromise = this.plexPost(statusUrl).catch((error) => {
